#!/usr/bin/python3
"""This script parses .go source files and extracts Prometheus metric names
and descriptions."""
...
def parse_file(path):
    """Parses given file and adds all metrics found to metrics."""
    ...
    with open(path, "r") as file:
        for ln, line in enumerate(file):
            ...
            if re.search("[a-zA-Z]+\\s+=\\s+\"[a-zA-Z_]+\"\\s*//.*" +
                         MAGIC_STRING, line):
                ls = line.split()
                m[ls[0]] = {
                    "name": ls[2][1:-1],
