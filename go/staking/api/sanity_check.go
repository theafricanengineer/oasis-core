...
func (g *Genesis) SanityCheck(now epochtime.EpochTime) error { // nolint: gocyclo
	...
	for id, delegations := range g.Delegations {
		...
		err := SanityCheckDelegations(id, acct, delegations)
		if err != nil {
			return err
		}
	}
	...
	for id, delegations := range g.DebondingDelegations {
		...
		err := SanityCheckDebondingDelegations(id, acct, delegations)
		if err != nil {
			return err
		}
	}
	...
	for id, acct := range g.Ledger {
		err := SanityCheckAccountShares(id, acct, g.Delegations[id], g.DebondingDelegations[id])
		if err != nil {
			return err
		}
	}
